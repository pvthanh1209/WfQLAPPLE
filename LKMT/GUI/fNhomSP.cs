...
namespace LKMT.GUI
{
    public partial class fNhomSP : UserControl
    {
        public fNhomSP()
        {
            ...
            dgvNhomSP.Columns[1].HeaderText = "Tên nhóm sản phẩm";
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
           ...
           else if(!NhomSanPhamBUS.Instance.ckMaSp(txtID.Text))
                MessageBox.Show("Mã sản phẩm đã bị trùng!!", "Thông Báo", MessageBoxButtons.OK);
            else
            {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (selectedRowCount >= 1)
            {
                ...
                {
                    MessageBox.Show("Xóa nhóm sản phẩm thành công!!", "Thông Báo", MessageBoxButtons.OK);
                    NhomSanPhamBUS.Instance.showNhomSP(dgvNhomSP);
                    btnLamMoi_Click(sender, e);
                }
                else MessageBox.Show("Không thể xóa nhóm sản phẩm này, vì còn tồn tại loại sản phẩm hoặc thương hiệu thuộc nhóm này!!", "Thông Báo", MessageBoxButtons.OK);
                btnLamMoi_Click(sender, e);
            }
            else
            {
                MessageBox.Show("Vui lòng chọn nhóm sản phẩm muốn xóa!!", "Thông Báo", MessageBoxButtons.OK);
            }
        }
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            if (selectedRowCount > 0)
            {
                ...
                {
                    MessageBox.Show("Cập nhật nhóm sản phẩm thành công!!", "Thông Báo", MessageBoxButtons.OK);
                    NhomSanPhamBUS.Instance.showNhomSP(dgvNhomSP);
                    btnLamMoi_Click(sender, e);
                }
                else MessageBox.Show("Cập nhật nhóm sản phẩm thất bại!!", "Thông Báo", MessageBoxButtons.OK);
            }
            else
            {
                MessageBox.Show("Vui lòng chọn nhóm sản phẩm muốn cập nhật!!", "Thông Báo", MessageBoxButtons.OK);
            }         
        }
        private void dgvNhomSP_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (selectedRowCount >0)
            {
                if (e.RowIndex != -1)
                {
                    ...
                    txtNgayTao.Text =  DateTime.Parse(row.Cells[2].Value.ToString()).ToString("yyyy/MM/dd");
                    txtNgayCapNhat.Text = DateTime.Parse(row.Cells[3].Value.ToString()).ToString("yyyy/MM/dd");
                }            
            }
        }
